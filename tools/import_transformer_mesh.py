...
def export(source, destination):
    ...
    with np.load(mesh_path, allow_pickle=False) as m, np.load(flow_path, allow_pickle=False) as f:
        ...
        locations = np.concatenate((nodes[cells],
                                   ((nodes[cells[:, 0]] + nodes[cells[:, 1]]) / 2)[:, None],
                                   ((nodes[cells[:, 0]] + nodes[cells[:, 2]]) / 2)[:, None],
                                   ((nodes[cells[:, 1]] + nodes[cells[:, 2]]) / 2)[:, None]), axis=1)
        ...
        oil_capacity = float(np.interp(reference_C, m["oil_T_C"], m["oil_rho"])
        ...
        physical = {
            ...
            "capacity_J_m3_K": [oil_capacity, float(m["rhoCv_solid"]),
                                 float(m["rhoCv_baffle"])],
        ...
        write_fields(destination / "inputs.npz", velocity_P2_m_s=velocity,
                     source_W_m3=m["cell_q_density"],
                     boundary_outlet=m["boundary_outlet"], boundary_wall=m["boundary_wall"])
        ...
        for key in ("boundary_inlet", "boundary_outlet"):
            ...
            fluxes.append(float(np.sum(2 * np.pi * np.diff(points[:, 0]) / 6 * (
                points[:-1, 0] * f["v"][idx[:-1]] + 4 * mid[:, 0] * f["v"][imid]
        ...
        write_report(destination / "parameters.json", {
            "schema": "mesh-cht-input-v1", "geometry": "transformer_2d",
            ...
            "physical": physical,
            "flow_verification": {"inlet_outlet_relative_imbalance": imbalance,
