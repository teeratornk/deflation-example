...
def controls(config):
    ...
    for key in ("level",):
        integer(c[key], key)
    for key in ("slabs", "targets", "rank", "repeats", "threads", "inner_cap", "outer_cap"):
        integer(c[key], key, 1)
...
def sequence(c, method, torch=None, api=None):
    ...
    try:
        ...
        if method != "direct":
            adapter = StudySolver(method, device=c["device"], rank=c["rank"], window=c["rank"],
                                   ...
                                   refresh=1000, torch=torch, api=api)
    ...
    except (ValueError, RuntimeError, MemoryError, np.linalg.LinAlgError) as error:
...
def worker(specification, output, method, repetition):
    ...
    if c["device"] == "cuda":
        ...
        torch.ones(8, dtype=torch.float64, device="cuda").sum().item()
