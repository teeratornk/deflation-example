...
def generate(output, size=0.12):
    ...
    try:
        ...
        bore = occ.addCylinder(.5, .5, .12, 0, 0, .66, .3)
        ...
        boundary = np.flatnonzero(np.any(np.isclose(nodes, 0, atol=1e-12)
                                         | np.isclose(nodes, 1, atol=1e-12), axis=1))
        mesh = ThermalMesh(nodes, orient_cells(nodes, np.vstack(cells)),
                           np.asarray(materials), boundary)
        mesh.save(output / "mesh.npz")
        write_report(output / "parameters.json", {
            "schema": "mesh-cht-input-v1", "geometry": "engine_3d",
            "generator": "deflation_example.generate_engine_mesh", "gmsh": gmsh.__version__,
            "mesh_size": size, "mesher_threads": 1,
            "coordinates": "(x1,x2,x3) divided by block length",
            "geometry_parameters": {"block": [1., 1., 1.], "bore_center": [.5, .5],
                                    "bore_radius": .3, "floor": .12, "deck": .78},
            "materials": {"0": "gas", "1": "metal"},
            "physical": {"conductivity_ratio": 1000., "metal_conductivity_W_m_K": 55.,
                         "inward_interface_flux_W_m2": 500000., "length_scale_m": .1,
                         "coolant_temperature_K": 360.,
                         "temperature_scale_K": 500000. * .1 / 55.,
                         "dimensionless_interface_flux": 1000.,
                         "dimensionless_capacity": [1., 1.],
            ...
            "source": {"manuscript_section": "03c_engine.tex",
