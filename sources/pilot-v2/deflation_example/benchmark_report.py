...
def checked_series(report):
    ...
    for grid in report["grids"]:
        ...
        for method in METHODS:
            ...
            reference = grid["reference_seconds"] if method == "deflated_gpu_qr" else 0.0
            session = (
                sum(
                    grid[k]["total_seconds"]
                    for k in ("persistent_session_startup", "persistent_session_cleanup")
                )
                if method == "amgx_persistent_resources"
                else 0.0
            )
            ...
            for case in cases:
                ...
                for run in runs:
                    ...
                    if set(components) != set(PHASES) or any(
                        not math.isfinite(v) or v < 0 for v in components.values()
                    ):
                        raise ValueError("Invalid timing components")
                    if not math.isclose(
                        sum(components.values()), run["total_seconds"], rel_tol=1e-12, abs_tol=1e-12
                    ):
                ...
                if not any(
                    r["components_seconds"] == selected["components_seconds"] for r in matching
                ):
...
def plot(series, destination):
    ...
    with plt.rc_context({"font.size": 8, "axes.titlesize": 9}):
        fig, axes = plt.subplots(
            2, len(series), figsize=(7.2, 5.2), squeeze=False, layout="constrained"
        )
        for column, (n, data) in enumerate(series.items()):
            ...
            for j, (method, label, color) in enumerate(zip(METHODS, LABELS, COLORS)):
                ...
                for index, trials in enumerate(values["repetition_seconds"], 1):
                    center = index + (j - 1) * 0.2
                    bottom.scatter(
                        [center + (k - (len(trials) - 1) / 2) * 0.02 for k in range(len(trials))],
                        [1000 * t for t in trials],
                        color=color,
                        s=12,
                        alpha=0.8,
                    )
                    bottom.plot(
                        [center - 0.06, center + 0.06],
                        [1000 * median(trials)] * 2,
                        color=color,
                        lw=2,
                    )
            top.set(
                title=f"{n} x {n}",
                xlabel="Completed instances",
                ylabel="Cumulative cost (s)",
                xticks=range(4),
                ylim=(0, None),
            )
            bottom.set(
                xlabel="Instance", ylabel="Kernel time (ms)", xticks=(1, 2, 3), xlim=(0.5, 3.5)
            )
