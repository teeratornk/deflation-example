...
from deflation_example.meshes import (
    ThermalMesh, assemble_thermal, simplex_geometry, orient_cells, add_facet_load,
)
...
def rectangle(axisymmetric=False):
    ...
    return ThermalMesh(x, np.array([[0, 1, 3], [0, 3, 2]]),
                       np.array([0, 1]), np.array([0, 1]), axisymmetric)
...
def test_exact_affine_field_integrals(axisymmetric):
    ...
    affine = mesh.nodes @ np.array([2., -3.]) + 4
    np.testing.assert_allclose(np.einsum("ei,eid->ed", affine[mesh.cells], grad),
                               np.tile([2., -3.], (2, 1)), atol=1e-13)
    a = assembly(mesh)
    ...
    add_facet_load(a, np.array([[0, 1]]), 2.)
    np.testing.assert_allclose(a.load.sum(), 6 * np.pi if axisymmetric else 2.)


@pytest.mark.parametrize("dim", [2, 3])
def test_nested_refinement_preserves_geometry_and_linear_fields(dim):
    coarse = rectangle(True) if dim == 2 else ThermalMesh(
        np.array([[0., 0., 0.], [1., 0., 0.], [0., 1., 0.], [0., 0., 1.]]),
        np.array([[0, 1, 2, 3]]), np.array([1]), np.array([0, 1, 2]))
...
def test_quadratic_velocity_integral_is_preserved_under_refinement():
    ...
    points = np.concatenate((x, ((x[:, 0]+x[:, 1])/2)[:, None],
                             ((x[:, 0]+x[:, 2])/2)[:, None],
                             ((x[:, 1]+x[:, 2])/2)[:, None]), axis=1)
    ...
    np.testing.assert_allclose((P.T @ af.transport @ P).toarray(), a.transport.toarray(),
...
def test_complete_optimization_matches_independent_bvls(steps, matrix_free):
    ...
    a = assembly(mesh)
    problem = build_mesh_control(a, time_steps=steps, initial=np.full(len(mesh.free), 0.07),
                                 boundary_value=0.1, assembled_restriction=not matrix_free)
    ...
    desired = rng.uniform(-0.5, 1., problem.size)
    bound = .12
    root = np.sqrt(problem.weights)
    factor = np.vstack((np.diag(root), np.sqrt(problem.alpha)*root[:, None]*problem.A.toarray()))
    ...
    reference = build_mesh_reference(problem, assembly(rectangle(True)),
                                     P[mesh.free][:, rectangle(True).free], 1)
    adapter = StudySolver("reference", reference=reference, rank=1, cg_factor=0.1)
    result = pdas(problem.H, problem.load(desired), bound,
                  initial_active=np.ones(problem.size, dtype=bool), tolerance=1e-9,
    ...
    h = 1e-5
    difference = (problem.objective(x+h*p, desired)-problem.objective(x-h*p, desired))/(2*h)
    np.testing.assert_allclose(difference, problem.objective_scale * g @ p, rtol=1e-9)
    multiplier = np.where(independent.active_mask == 1,
                          problem.load(desired)-problem.H @ independent.x, 0.)
    assert max(kkt_metrics(problem.H, problem.load(desired), bound,
                           independent.x, multiplier).values()) < 1e-9
...
def test_restriction_keeps_active_state_equation_rows():
    ...
    wrong = (sparse.diags(problem.weights[I]) + problem.alpha * problem.A[I][:, I].T
             @ sparse.diags(problem.weights[I]) @ problem.A[I][:, I]).toarray()
