...
def exporter():
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            "mesh_export", directory / "export_mesh_records.py"
        )
...
def test_manifest_rejects_unlisted_files_and_path_traversal(tmp_path):
    ...
    manifest = {
        "format": "mesh-cht-evidence-v1",
        "files": {"record.json": {"bytes": len(data), "sha256": hashlib.sha256(data).hexdigest()}},
    }
...
def test_memory_envelope_uses_all_repetitions_and_keeps_rank_zero_fixed(tmp_path):
    ...
    for rank in (20, 100, 200):
        methods = [
            {
                ...
                "accepted": 5,
                "requested": 5,
                "complete_memory_measurements": 5,
                "median_seconds": 1000 / rank,
        ...
        studies.append(
            {
                "controls": {"geometry": "engine_3d", "level": 2, "transient": False, "rank": rank},
                "methods": methods,
            }
        )
