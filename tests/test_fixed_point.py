...
@pytest.mark.parametrize("method", ["picard", "anderson"])
def test_momentum_policy_preserves_control_and_exact_derivatives(method):
    problem = small_coupled_problem([0.2, 0.35], uniform_capacity=True)
    state = np.full(problem.size, 0.015)
    old = problem.evaluate(state)
    problem.flow_method = method
    current = problem.evaluate(state)
    np.testing.assert_allclose(current.control, old.control, rtol=1e-7, atol=1e-9)
    direction = np.random.default_rng(23).normal(size=problem.size)
    np.testing.assert_allclose(
        current.jacobian @ direction, old.jacobian @ direction, rtol=1e-7, atol=1e-9
    )
    epsilon = 1e-5
    plus = problem.evaluate(state + epsilon * direction, initial=current)
    minus = problem.evaluate(state - epsilon * direction, initial=current)
    np.testing.assert_allclose(
        (plus.control - minus.control) / (2 * epsilon),
        current.jacobian @ direction,
        rtol=1e-5,
        atol=1e-7,
    )
