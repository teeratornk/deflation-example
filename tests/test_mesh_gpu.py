...
def test_resident_space_time_restriction_matches_host(torch_cuda):
    ...
    reference = SpaceTimeReference(rng.normal(size=(10, 2)), rng.normal(size=(4, 3)), [0, 1, 1], {})
    ...
    B = sparse.diags([-np.ones(19), np.full(20, 3.0), -np.ones(19)], [-1, 0, 1], format="csr")
...
def test_resident_recycling_transfers_existing_coarse_vectors(torch_cuda):
    ...
    B = sparse.diags([-np.ones(39), np.full(40, 3.0), -np.ones(39)], [-1, 0, 1], format="csr")
    host = StudySolver("recycling", device="cuda", rank=5, window=5, torch=torch_cuda)
    device = StudySolver(
        "recycling", device="cuda", rank=5, window=5, torch=torch_cuda, resident_recycling=True
    )
    try:
        for shift in (0, 2, 1):
