...
def complete_sequence(n, rank, bound, method, torch, api, rtol, outer_tolerance):
    ...
    reference_seconds, resource_seconds = 0., 0.
    cases = []
    try:
            ...
            def solve(B, b, I):
                ...
                if Phi is not None:
                    result, metrics = gpu_deflated_cg(
                        B, b, Phi[I], B.diagonal(), rtol=rtol,
                        maxiter=10000, basis_backend="gpu_qr")
                else:
                    result, metrics = amgx_cg(
                        B, b, api=api, synchronize=torch.cuda.synchronize,
                        session=session, rtol=rtol)
                inner.append({**metrics, "callback_seconds": time.perf_counter()-begin,
                    "n_inactive": len(I), "iterations": result.iterations, "rank": result.rank,
                    "original_residual": result.residual, "status": result.status,
                    "coarse_condition": result.coarse_condition, "fallback_reason": result.fallback_reason})
                return result

            outer = pdas(problem.H, f, bound, tolerance=outer_tolerance, linear_solver=solve)
            case = {"theta": theta, **summarize_pdas(outer, time.perf_counter()-tick), "inner": inner}
            cases.append(case)
    ...
    return {"method": method, "total_seconds": total, "assembly_seconds": assembly_seconds,
        "reference_seconds": reference_seconds, "resource_creation_seconds": resource_seconds,
        "cleanup_seconds": cleanup_seconds, "cases": cases,
...
def run(output, n=24, rank=100, repeats=5, rtol=1e-10, outer_tolerance=1e-8, threads=4):
    for value, name in [(n, "Grid"), (rank, "Rank"), (repeats, "Repetitions"), (threads, "Threads")]:
    ...
    with threadpool_limits(limits=threads):
        ...
        bound, fraction = calibrate_bound(calibration_problem.H, calibration_problem.target(np.pi/4), .2, steps=8)
        ...
        report = {"protocol": "complete-cht-pdas-v1", "n": n, "rank": rank, "repeats": repeats,
            "rtol": rtol, "outer_tolerance": outer_tolerance, "bound": bound,
            "calibrated_activity": fraction, "calibration_seconds": calibration_seconds,
            "shared_runtime_seconds": runtime_seconds, "initial_active": "empty for each target",
            ...
            "hierarchy_reused": False, "configuration": amgx_configuration(rtol),
            "environment": {**environment(), "torch": torch.__version__, "cuda": torch.version.cuda,
                            "gpu": torch.cuda.get_device_name()}, "sequences": []}
        try:
            for repeat in range(repeats):
                ...
                for method in methods:
                    sequence = complete_sequence(n, rank, bound, method, torch, pyamgx, rtol, outer_tolerance)
                    report["sequences"].append({"repetition": repeat, **sequence})
                    write_report(output / "results.json", report)
                    print(f"{method}, repeat {repeat}: success={sequence['success']}, "
                          f"seconds={sequence['total_seconds']:.4f}, "
                          f"outer={sequence['outer_iterations']}, inner={sequence['inner_iterations']}", flush=True)
