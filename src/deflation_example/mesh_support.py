...
def run(validation, transfers, pilots, output):
    ...
    for c in checked["cases"]:
        metrics = [*c["independent_kkt"].values(), *c["pdas_kkt"].values(),
                   c["state_relative_difference"], c["objective_relative_difference"],
                   c["forward_recovery_relative"], c["adjoint_action_relative"]]
        if not np.isfinite(metrics).all() or min(metrics) < 0:
            raise ValueError("Verification metrics must be finite and nonnegative")
        if (not c["success"] or not c["independent_success"] or c["pdas_status"] != "converged"
            or max(c["independent_kkt"].values()) > 1e-8 or max(c["pdas_kkt"].values()) > 1e-8
            or c["state_relative_difference"] > 1e-7
            or c["forward_recovery_relative"] > 1e-8 or c["adjoint_action_relative"] > 1e-12):
        ...
        rows.append([name, kind, c["query"]+1, c["dimension"],
                     f"{c['state_relative_difference']:.2e}", f"{c['objective_relative_difference']:.2e}",
    ...
    for name, path in transfers.items():
        ...
        for method, label in (("full_reference", "Full reference"), ("sequential_transfer", "Sequential transfer")):
            data = [r["methods"][method] for r in selected]
            repetitions = [p for v in data for p in v["repetitions"]]
            for p in repetitions:
                if not np.isfinite([p["original_residual"], p["seconds"],
                                    p["basis_transfer_seconds"], p["kernel_seconds"]]).all():
                    raise ValueError("Transfer metrics must be finite")
                if p["status"] != "converged" or p["original_residual"] > source["source_controls"]["rtol"]:
                    raise ValueError("A transfer repetition fails residual acceptance")
                if not np.isclose(p["seconds"], p["basis_transfer_seconds"]+p["kernel_seconds"], rtol=1e-12):
            ...
            record = {"study": name, "method": method, "trace_kernels": len(source["rows"]),
                      "sampled_kernels": len(selected), "repetitions": len(repetitions),
                      "rank_range": [min(ranks), max(ranks)],
                      "median_energy_removed": float(np.median([v["correction"]["coarse_removed_energy_fraction"] for v in data])),
                      "summed_median_iterations": sum(float(np.median([p["iterations"] for p in v["repetitions"]])) for v in data),
                      "summed_median_seconds": sum(float(np.median([p["seconds"] for p in v["repetitions"]])) for v in data),
    ...
    for name, directory in pilots.items():
        ...
        for method in protocol["methods"]:
            record = read(directory / f"{method}-0/record.json", f"pilot/{name}/{method}-0/record.json")
            accepted = sum(c["status"] == "converged" for c in record.get("cases", []))
            statuses = sorted({c["status"].removeprefix("inner_") for c in record.get("cases", []) if c["status"] != "converged"})
    ...
    report = {"protocol": "mesh-support-tables-v1", "generator_environment": environment(),
              "input_sha256": inputs, **summaries}
