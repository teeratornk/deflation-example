...
def transformer_subproblem(assembly):
    ...
    """
    selected = np.flatnonzero(assembly.mesh.nodes[:, 1] < 0.04)
    ...
    pins = np.flatnonzero(
        np.any(np.isclose(nodes, nodes.min(axis=0)) | np.isclose(nodes, nodes.max(axis=0)), axis=1)
    )
    ...
    return ThermalAssembly(
        mesh,
        assembly.mass[selected],
        assembly.capacity[selected],
        restrict(assembly.diffusion),
        restrict(assembly.transport),
        restrict(assembly.stabilization),
        assembly.load[selected],
    )
...
def validate(settings="physical"):
    ...
    for geometry, alpha, bound in (("engine_3d", 1e-6, 0.2), ("transformer_2d", 1e-14, 0.8)):
        ...
        horizon = (
            0.1
            if geometry == "engine_3d"
            else 600 / showcase.parameters["physical"]["time_scale_s"]
        )
        ...
        for steps in (None, [0.5 * horizon, 0.5 * horizon], [0.2 * horizon, 0.8 * horizon]):
            model = build_mesh_control(
                a, alpha, time_steps=steps, initial=np.full(len(a.mesh.free), 0.03)
            )
            ...
            for query in range(2):
                ...
                def solve(B, b, I):
                    ...
                    return LinearResult(
                        x, 1, residual, "converged" if residual <= 1e-10 else "residual_failed"
                    )
                ...
                adjoint_error = relative_norm(
                    model.apply_transpose(trial) - model.A.T @ trial, model.A.T @ trial
                )
                success = (
                    reference.success
                    and result["status"] == "converged"
                    and max(kkt.values()) <= 1e-8
                    and relative <= 1e-7
                    and forward_error <= 1e-8
                    and adjoint_error <= 1e-12
                )
                rows.append(
                    {
                        "geometry": geometry,
                        ...
                        "scope": "full engine mesh"
                        if geometry == "engine_3d"
                        else "transformer inlet-region principal subproblem",
                        "independent_success": bool(reference.success),
                        "independent_kkt": kkt,
                        "pdas_status": result["status"],
                        "pdas_kkt": result["kkt"],
                        "state_relative_difference": relative,
                        "objective_relative_difference": abs(
                            model.objective(result["y"], desired)
                            - model.objective(reference.x, desired)
                        )
                        / max(model.objective(reference.x, desired), 1e-30),
    ...
    return {
        "protocol": "mesh-cht-independent-validation-v2",
        "settings": settings,
