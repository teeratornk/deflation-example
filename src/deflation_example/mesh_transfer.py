...
def run(record_path, output, device="cpu", samples=8, repeats=3):
    ...
    trace = [
        (case["query"], j, row) for case in source["cases"] for j, row in enumerate(case["inner"])
    ]
    if not trace:
    ...
    if device == "cuda":
        ...
        require_cuda()
        warmed, _ = gpu_deflated_cg(
            sparse.diags(np.arange(1.0, 17), format="csr"),
            np.ones(16),
            np.eye(16)[:, :2],
        ...
        def solve(B, b, basis):
            return gpu_deflated_cg(
                B,
                b,
                basis,
                B.diagonal(),
                basis_backend="gpu_qr",
                rtol=c["rtol"] * c["cg_factor"],
    ...
    result = {
        "protocol": "mesh-reference-transfer-v1",
    ...
    for index, (query, outer, row) in enumerate(trace):
        ...
        if previous is None:
            ...
            released, activated, identity_error = 0, 0, 0.0
        else:
        ...
        entry = {
            ...
            "query": query,
            "outer_step": outer,
            "inactive_count": len(I),
            "newly_inactive": released,
        ...
        if index in selected:
            ...
            if (
                ...
                != source["cases"][query]["target_sha256"]
            ):
            ...
            for repetition in range(repeats):
                ...
                for name in order:
                    ...
                    basis = (
                        ...
                        if name == "full_reference" or previous is None
            ...
            for name, basis in (("full_reference", full), ("sequential_transfer", sequential)):
                ...
                entry["methods"][name] = {
                    ...
                    "correction": correction_diagnostics(
                        B, b, basis, records[-1]["deployed_rank"], exact
                    ),
                }
