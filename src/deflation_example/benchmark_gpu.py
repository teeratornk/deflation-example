...
def problem_instances(n):
    ...
    A = laplacian(n)
    H = (sparse.eye(n*n) + .001 * (A.T @ A)).tocsr()
    x, y = np.meshgrid(np.arange(1, n+1)/(n+1), np.arange(1, n+1)/(n+1), indexing="ij")
    x, y = x.ravel(), y.ravel()
    for index, theta in enumerate((0., .35, .7)):
        f = np.exp(-((x-.5-.15*np.cos(theta))**2 + (y-.5-.15*np.sin(theta))**2)/.025)
        if index == 0:
            bound, _ = calibrate_bound(H, f, .2, steps=8)
        ...
        yield B, b, I, {
            "n": n, "index": index, "theta": theta, "bound": bound, "n_inactive": len(I),
            "mask_sha256": digest(I), "rhs_sha256": digest(b),
            "matrix_data_sha256": digest(B.data), "outer_kkt": outer["kkt"],
            "outer_status": outer["status"], "outer_seconds": outer_seconds,
...
def measured_call(function):
    ...
    return {**metrics, "iterations": result.iterations, "status": result.status,
            "residual": result.residual, "rank": result.rank,
...
def run_benchmark(output, grids=(64, 128), rank=100, repeats=5, threads=4, include_cpu=True):
    ...
    try:
        ...
        raise RuntimeError("Install AmgX and pyamgx as described in docs/gpu-benchmark.md") from error
    output = Path(output)
    output.mkdir(parents=True, exist_ok=False)
    torch.set_num_threads(threads)
    report = {
        "protocol": "itemized-gpu-resource-control-v1", "success": False,
        "controls": {"grids": grids, "rank": rank, "repeats": repeats, "threads": threads,
                     "rtol": 1e-10, "outer_tolerance": 1e-8, "maxiter": 10000,
                     "jacobi_maxiter": 20000,
                     "rank_threshold": 1e-12, "coarse_condition_limit": 1e10,
                     "initial_guess": "zero", "residual_refresh": 1000,
                     "hierarchy_reuse": False, "include_cpu": include_cpu},
        "amgx_configuration": amgx_configuration(), "cases": [], "grids": [],
        ...
        with threadpool_limits(limits=threads):
            ...
            report["environment"] = {
                **environment(), "torch": torch.__version__, "cuda": torch.version.cuda,
                "gpu": torch.cuda.get_device_name(), "amgx_api": pyamgx.get_api_version(),
                "pyamgx_binary_sha256": hashlib.sha256(Path(pyamgx.__file__).read_bytes()).hexdigest(),
            }
            try:
            ...
            for n in grids:
                ...
                grid = {"n": n, "reference_seconds": time.perf_counter()-start,
                        "reference_sha256": digest(Phi), "mode_indices": modes, "warmups": {}}
                    ...
                    for B, b, I, case in problem_instances(n):
                        ...
                        methods = {
                            ...
                            "amgx_persistent_resources": lambda: amgx_cg(
                                B, b, api=pyamgx, synchronize=torch.cuda.synchronize, session=session),
                        }
                        if case["index"] == 0:
                        ...
                        case["solvers"] = {name: representative(runs)
                                           for name, runs in case["repetitions"].items()}
                        ...
                        print(f"Grid {n}, instance {case['index']}: " + ", ".join(
                            f"{key}={value['status']}" for key, value in case["solvers"].items()), flush=True)
                finally:
                    timer = PhaseTimer()
                    session.close()
                    timer.mark("cleanup")
            ...
            report["success"] = all(
                r["status"] == "converged"
                for c in report["cases"] for r in c["solvers"].values()
            ) and all(r["status"] == "converged" for g in report["grids"] for r in g["warmups"].values())
    finally:
        if initialized:
            start = time.perf_counter()
            pyamgx.finalize()
...
def main():
    if any(arg == "-m" or (arg.startswith("--") and len(arg) > 2 and "--multirun".startswith(arg))
