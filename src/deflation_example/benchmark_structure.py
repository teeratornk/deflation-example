...
def restriction_example():
    ...
    """
    Q = np.eye(8)
    for a, b, angle in [(0, 7, 0.2), (1, 7, 0.15), (0, 2, 0.1)]:
        rotation = np.eye(8)
        rotation[a, a] = rotation[b, b] = np.cos(angle)
        rotation[a, b], rotation[b, a] = -np.sin(angle), np.sin(angle)
        Q = Q @ rotation
    M0 = (Q * np.array([1.0, 2.0, 5.0, 8.0, 11.0, 14.0, 17.0, 20.0])) @ Q.T
    Phi, I, J = Q[:, :2], np.arange(7), np.array([7])
    W = Phi[I]
    Z = orthonormalize(W)
    v = Z[:, 1]
    rows = []
    for amplitude in (0.0, 0.01, 0.1, 0.5, 1.0, 2.0, 4.0, 8.0):
        ...
        direct_bound = min(1.0, linalg.norm(F, "fro") / eta) if eta > 0 else None
        sufficient = min(1.0, numerator / eta) if eta > 0 else None
        residual_identity = (
            (np.eye(7) - Z @ Z.T) @ (-M0[np.ix_(I, J)] @ Phi[J] + delta @ W) @ linalg.inv(Z.T @ W)
        )
        rows.append(
            {
                "amplitude": amplitude,
                ...
                "removed_mass": beta,
                "coupling_frobenius": coupling,
                "subspace_residual_frobenius": linalg.norm(F, "fro"),
                "residual_identity_error": linalg.norm(F - residual_identity, "fro"),
                "separation": eta,
                "largest_angle_degrees": np.rad2deg(angle),
                "actual_sine": np.sin(angle),
                "residual_angle_bound": direct_bound,
    ...
    return {
        ...
        "full_dimension": 8,
        "inactive_indices_zero_based": I,
        "reference_eigenvalues": [1.0, 2.0, 5.0, 8.0, 11.0, 14.0, 17.0, 20.0],
        "rotations": [[0, 7, 0.2], [1, 7, 0.15], [0, 2, 0.1]],
        "rows": rows,
    }
...
def ritz_comparison(n=12, pool=300, rank=100, repeats=5):
        ...
        for repeat in range(repeats):
            ...
            for method in order:
                ...
                Z = (
                    candidates[:, :rank]
                    if method == "reference"
                    else ritz_space(B, candidates, rank, method == "scaled_ritz")
                )
    ...
    return {
        ...
        "rows": rows,
        "success": all(r["status"] == "converged" for r in rows),
    }
