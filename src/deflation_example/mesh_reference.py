...
class DeviceMeshReference:
    """Retain reference factors on the GPU and form each restriction there."""

    def __init__(self, reference, torch):
        self.host = reference
        self.torch = torch
        self.rank, self.description = reference.rank, reference.description
        self.temporal = None
        if isinstance(reference, SpaceTimeReference):
            self.spatial = torch.tensor(reference.spatial, dtype=torch.float64, device="cuda")
            self.temporal = torch.tensor(reference.temporal, dtype=torch.float64, device="cuda")
            self.columns = torch.tensor(reference.spatial_columns, device="cuda")
        else:
            self.spatial = torch.tensor(reference.basis, dtype=torch.float64, device="cuda")
        torch.cuda.synchronize()

    def restrict(self, indices):
        return self.host.restrict(indices)

    def restrict_device(self, indices):
        rows = self.torch.tensor(indices, device="cuda")
...
def build_mesh_reference(
    ...
    if np.any(norms == 0):
        raise ValueError("Prolongation lost an entire reference direction")
    phi = phi / norms
    description = {
