...
def controls(config):
    ...
    for key in ("slabs", "targets", "rank", "repeats", "threads", "inner_cap", "outer_cap", "residual_refresh"):
        integer(c[key], key, 1)
    for key in ("horizon", "alpha", "rtol", "kkt_tolerance", "cg_factor", "amgx_factor", "timeout_seconds"):
    ...
    if c["reference_device"] not in {"cpu", "cuda"} or (c["reference_device"] == "cuda" and c["device"] != "cuda"):
    ...
    if c["phase"] == "final" and (c["repeats"] < 5 or c["targets"] < 16):
        raise ValueError("Final populations require at least 16 targets and five complete repetitions")
    return c
...
def build_model(c):
    ...
    model = build_mesh_control(showcase.assembly, alpha=c["alpha"], time_steps=steps,
                               initial=initial, assembled_restriction=not c["matrix_free_inner"])
...
def sequence(c, method, torch=None, api=None):
    ...
    if c["monitor_memory"]:
        ...
        try:
            ...
            return ({"success": False, "failure": {"status": "preparation_failed",
                     "stage": "memory_initialization", "error_type": type(error).__name__,
                     "measurement_error_type": getattr(sampler, "initial_state", {}).get("error")},
                     "cases": []}, [], None)
    synchronize()
    ...
    try:
        ...
        storage["state_operator_bytes"] = sum(a.nbytes for a in (
            problem.A.data, problem.A.indices, problem.A.indptr))
        tick = time.perf_counter()
        if method == "reference":
            reference = build_mesh_reference(problem, showcase.coarse_assembly,
                                             showcase.prolongation, c["rank"], c["construction"],
                                             c["spatial_reference"], c["temporal_metric"])
        ...
        for query in range(c["targets"]):
            ...
            active = previous["active"] if previous is not None and warm else np.ones(problem.size, bool)
            state = previous["y"].copy() if previous is not None and warm else np.zeros(problem.size)
            inner = []
            ...
            def solve(B, b, I):
                ...
                if method == "direct":
                    ...
                    solved = LinearResult(y, 1, residual,
                                           "converged" if residual <= c["rtol"] else "residual_failed")
                ...
                mask[I] = True
                inner.append({**timing, "status": solved.status, "iterations": solved.iterations,
                              "original_residual": solved.residual, "deployed_rank": solved.rank,
                              "coarse_condition": solved.coarse_condition,
                              "fallback_reason": solved.fallback_reason,
                              "inactive_mask_bits": pack_mask(mask), "rhs_sha256": _hash(b),
                              "initial_sha256": _hash(np.zeros(len(I)) if initial is None else initial)})
                return solved

            solved = pdas(problem.H, load, c["bound"], initial_active=active,
                          tolerance=c["kkt_tolerance"], maxiter=c["outer_cap"], linear_solver=solve)
            control, adjoint = problem.recover(solved["y"])
            recovery = relative_norm(problem.weights*(solved["y"]-desired)
                                     + problem.apply_transpose(adjoint) + solved["multiplier"], load)
            status = solved["status"]
            if status == "converged" and (not np.isfinite(recovery) or recovery > c["kkt_tolerance"]):
                status = "recovery_failed"
            if status == "converged" and any(row["original_residual"] > c["rtol"] for row in inner):
                status = "residual_failed"
            case = {"query": query, "status": status, "target_sha256": _hash(desired),
                    "solution_sha256": _hash(solved["y"]), "seconds": time.perf_counter()-tick,
                    "outer_iterations": solved["iterations"], "kkt": solved["kkt"],
                    "inner_iterations": sum(row["iterations"] for row in inner),
                    "outer_history": solved["history"], "outer_timing": solved["timing"],
                    "inner": inner, "active_count": int(solved["active"].sum()),
            ...
            if c["save_fields"]:
                fields.append({"state": solved["y"].copy(), "desired": desired.copy(),
                               "active": solved["active"].copy(), "control": control.copy(),
        ...
        failure = {"status": "memory_error" if isinstance(error, MemoryError) else "exception",
    ...
    return {"method": method, "seconds": seconds, "components_seconds": components,
            ...
            "timing_scope": "assembly, reference construction, all PDAS steps and inner solves, transfers, verification, solver cleanup; field serialization and common startup excluded"}, fields, mesh
...
def worker(specification, output, method, repetition):
    ...
    with threadpool_limits(c["threads"]):
        try:
            if c["device"] == "cuda":
                ...
                if "amgx" in c["methods"]:
                    ...
                    gpu_deflated_cg(sparse.eye(16, format="csr"), np.ones(16), np.eye(16)[:, :2],
            ...
            record = {"success": False, "failure": {"status": "worker_exception",
                                                     "error_type": type(error).__name__}}
        finally:
        ...
        record.update({"protocol": "mesh-cht-complete-v1", "controls": c, "method": method,
                       "repetition": repetition, "environment": environment(),
                       "startup_after_import_seconds": startup,
                       "finalization_seconds": finalization})
        if torch is not None:
            record["environment"].update(torch=torch.__version__, cuda=torch.version.cuda,
                                           gpu=torch.cuda.get_device_name())
        if initialized:
            from .amgx import amgx_configuration
            record["amgx_configuration"] = amgx_configuration(c["rtol"] * c["amgx_factor"],
                                                              c["inner_cap"], True)
        write_report(output / "record.json", record)
        if mesh is not None and fields:
            write_fields(output / "geometry.npz", nodes=mesh.nodes, cells=mesh.cells,
                         materials=mesh.materials, free=mesh.free)
...
def run(config):
    ...
    for repetition in range(c["repeats"]):
        order = c["methods"][repetition % len(c["methods"]):] + c["methods"][:repetition % len(c["methods"])]
        for method in order:
            directory = output / f"{method}-{repetition}"
            start = time.perf_counter()
            command = [sys.executable, "-m", "deflation_example.benchmark_mesh", "--worker",
                       str((output / "protocol.json").resolve()), "--output", str(directory.resolve()),
                       "--method", method, "--repetition", str(repetition)]
            try:
                result = subprocess.run(command, timeout=c["timeout_seconds"], check=False)
                status = "completed" if result.returncode == 0 else "failed"
            except subprocess.TimeoutExpired:
                status = "timeout"
            results.append({"method": method, "repetition": repetition, "status": status,
