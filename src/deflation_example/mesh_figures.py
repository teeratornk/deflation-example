...
def plot_showcases(transformer, engine, output, queries=(0, 7, 15), slab=1):
    ...
    for row, directory in enumerate((transformer, engine)):
        ...
        if row:
            ...
            shell = Poly3DCollection(coarse.nodes[unique[counts == 1]], facecolor="#66c2a5",
                                     edgecolor="#447765", linewidth=.2, alpha=.85)
            ...
            for a, b in combinations(cube, 2):
                if np.sum(np.abs(a-b)) == 1:
                    geometry.plot(*np.array([a, b]).T, color=".35", linewidth=.7)
            geometry.set(xlim=(0, 1), ylim=(0, 1), zlim=(0, 1),
                         xlabel="$x_1/L$", ylabel="$x_2/L$", zlabel="$x_3/L$",
                         title="Gas bore inside metal block", box_aspect=(1, 1, 1))
            ...
            im = geometry.tripcolor(tri, facecolors=materials, cmap=materials_cmap,
                                    norm=BoundaryNorm(np.arange(4)-.5, 3), rasterized=True)
            geometry.set(xlabel="$z$ (m)", ylabel="$r$ (m)", title="Oil, winding, and baffles")
            colorbar = fig.colorbar(im, ax=geometry, orientation="horizontal", ticks=[0, 1, 2], pad=.12)
        ...
        for col, query in enumerate(queries, 1):
            ...
            artist = axis.tripcolor(tri, sampled[:, 0], shading="gouraud", cmap="inferno",
                                    vmin=0, vmax=1.25, rasterized=True)
            if sampled[:, 1].min() < .5 < sampled[:, 1].max():
                axis.tricontour(tri, sampled[:, 1], levels=[.5], colors="white", linewidths=.25 if not row else .8)
            title = f"Target {query+1}"
            if c["transient"]:
                time = (selected_slab+1)*c["horizon"]/c["slabs"]
                title += f", $t={time*record['parameters']['physical']['time_scale_s']:.0f}$ s" if not row else f", $t={time:.3f}$"
